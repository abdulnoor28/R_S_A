...
long long modexp(long long base,long long e,long long mod)
{
    //long long is used to store the big values in result
    long long result=1;
    base=base % mod;
    while(e>0)
    {
        //if e value is odd, multiply result by base
        if(e%2==1)
        {
    ...
    //after executing this function successfully we return the result
    return result;
}
...
void decrypt()
{
    ...
    printf("your decrpted message is : %d",decrypted);
}
